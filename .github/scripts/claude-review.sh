...
# 创建审查提示词
cat > ${TMP_PREFIX}-review-prompt.txt <<EOF
...
cat "${DIFF_FILE}" >> ${TMP_PREFIX}-review-prompt.txt

# 使用 Claude Code headless 模式进行审查
# 注意：这里使用 --output-json 标志（如果支持）或通过 prompt 引导输出 JSON
claude --headless --model claude-sonnet-4-6 \
  --input ${TMP_PREFIX}-review-prompt.txt \
  --output ${TMP_PREFIX}-claude-review-raw.txt \
  2>&1 | tee ${TMP_PREFIX}-claude-review.log

# 从输出中提取 JSON（Claude 可能会输出额外的文本）
echo "📝 Extracting JSON from Claude response..."

# 尝试提取 JSON 对象
grep -o '{.*}' ${TMP_PREFIX}-claude-review-raw.txt | jq '.' > ${TMP_PREFIX}-claude-review-result.json || {
  echo "❌ Failed to extract valid JSON from Claude response"
  echo "Raw response:"
  cat ${TMP_PREFIX}-claude-review-raw.txt

  # 创建一个默认的失败响应
  cat > ${TMP_PREFIX}-claude-review-result.json <<FALLBACK
{
  "passed": false,
  "risk_level": "high",
  "can_auto_fix": false,
...
# 验证 JSON 格式
if jq empty ${TMP_PREFIX}-claude-review-result.json 2>/dev/null; then
  echo "✅ Valid JSON generated"
  echo "Review result:"
  cat ${TMP_PREFIX}-claude-review-result.json | jq '.'
else
  echo "❌ Invalid JSON format"
  exit 1
fi

# 将结果复制到标准位置供 workflow 使用
cp ${TMP_PREFIX}-claude-review-result.json /tmp/claude-review-result.json
